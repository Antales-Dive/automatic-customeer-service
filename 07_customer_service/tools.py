...
def _run_async_in_thread(coro):
    """在独立的事件循环中运行异步协程（线程安全）。

    当工具在异步上下文中被调用时，不能直接用 asyncio.run()
    （会报 running event loop 错误）。这个函数在单独的线程中
    创建新的事件循环来执行异步操作。
    """
    result = None
    exception = None
...
def make_transfer_human(session_id: str):
    ...
    def transfer_human(message: str) -> str:
        ...
        返回:
            工单编号和提示信息。
        """
        # 在独立线程的事件循环中运行 create_ticket，
        # 避免与 Agent ainvoke 的主事件循环冲突
        ticket = _run_async_in_thread(
            create_ticket(DB_PATH, session_id, message)
        )
